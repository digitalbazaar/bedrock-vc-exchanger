...
import * as schemas from '../schemas/bedrock-vc-exchanger.js';
import {
  schemas as bedrockSchemas,
  createValidateMiddleware as validate,
} from '@bedrock/validation';
import {
  createExchangeInstance, delegateZcap, getExchangeInstanceRecord,
  getStepIdByTransaction, initializeTransaction, storeResponseForExchangeStep
...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.post(
    routes.exchange,
    cors(),
    validate({bodySchema: schemas.initiateExchange()}),
    asyncHandler(async (req, res) => {
      const {exchangeId} = req.params;
      const stepId = 'initial';

      if(!(await exchange.exists({exchangeId}))) {
        throw new BedrockError(
          'Exchange not found.', 'NotFoundError', {
            httpStatusCode: 404,
            public: true
          });
      }
  ...
  app.put(
    ...
    cors(),
    validate({bodySchema: schemas.verifiablePresentation(bedrockSchemas)}),
    asyncHandler(async (req, res) => {
      const {exchangeId} = req.params;
      const {
        exchangeInstanceId,
        localTransactionId
      } = _parsePublicTransactionId({transactionId: req.params.transactionId});
      ...
      const nextStepId = await exchange.getNextStep(
        {exchangeId, currentStepId});
      const response = await exchange.generateResponse({
        exchangeId,
        exchangeInstanceId: exchangeInstance.id,
